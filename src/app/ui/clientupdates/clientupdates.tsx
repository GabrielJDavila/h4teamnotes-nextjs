import { fetchWorkoutNotes, fetchClientUpdates } from "@/app/lib/data";
import { EyeIcon } from "@heroicons/react/24/outline";
import Link from "next/link";
import { comfortaa } from "../fonts";
import clsx from "clsx";

export default async function ClientUpdates({
    query,
    currentPage
}: {
    query: string,
    ...
    const notes = await fetchClientUpdates(query, currentPage)
    const maxChars = 125
                    ...
                    return (
                        <div key={note.id} className="bg-gray-100 p-4 flex flex-col gap-8 rounded-md">
                            <div className="flex justify-between">
                                ...
                                <Link href={`/dashboard/clientupdates/${note.id}/edit`} className="flex gap-2 items-center rounded-lg p-2 cursor-pointer transition-colors hover:bg-gray-200">
